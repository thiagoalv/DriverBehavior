...
public class DTW {
     ...
     * @param template
     */
    public DTW(ArrayList<Float> sample, double[] template) {
        seq1 = sample;
        seq2 = template;
    ...
    public void compute() {
        ...
        // calculate distance between sensor data and template
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                d[i][j] = distanceBetween(seq1.get(i), seq2[j]);
            }
        }
